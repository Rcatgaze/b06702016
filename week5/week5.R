...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    tags$h2("     In our project, we use the data of MLB players in 200 .  In our expectation, the boss of the baseball team is anable to make a wise dicision on forming a team through the analysis.  First, we sort baseball players by position, and check the relationship between salary and position through regression analysis.
	The analysis gives players to find their career direction, especially for the senior-high players who have unlimited potential, they can choose a proper way to go on their career.
            ")
  ...
  tabPanel(
    "Single Variable",
    tags$h1("Single Variable"),
    tags$h2("The following is the explanation of each values :"),
    tags$h2("(1)Fr=Games played : low(Games played between 163~120), mid(Games played between 120~60), high(Games played between 60~0)"),
    tags$h2("(2)POS=Player's position  "),
    tags$img(height = 300, width = 300, src = "https://upload.wikimedia.org/wikipedia/commons/8/88/Baseball_positions.svg"),
    tags$h2("10 represents Pinch Hitter, 11 represents Pinch Runner"),
    tags$h2("(3)AVG=Batting average : Hit / At bats"),
    tags$h2("(4)OBP=On base percentage :Hit + Base Balled + Hit by pitch/（At bats + Base Balled + Hit by pitch + Sacrifice flies"),
    tags$h2("(5)SLG=Slugging percentage: Total bases / At bats"),
    tags$h2("(6)Pay=Salary : low(Under 5,000,000) mid(5,000,000~10,000,000) high(10,000,000~20,000,000) great(over 20,000,000)"),
    br(),
    sidebarLayout(
      sidebarPanel(
